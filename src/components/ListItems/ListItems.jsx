...
import {
    selectMyFavorites, selectQueryRecipes,
    selectUserRecipes,
} from '../../redux/recipes/recipesSelectors';
import {
    getFavorites,
    getUserRecipes, queryRecipes,
...
const ListItems = ({activeTab, userId, isOwnProfile}) => {
    ...
    const queryRecipesData = useSelector(selectQueryRecipes);
    ...
    useEffect(() => {
        if (activeTab === 'my-recipes') {
            ...
            !isOwnProfile && dispatch(queryRecipes({
                limit: 9,
                page: page,
                ownerId: userId,
            }));
    ...
    return (
        <div className={styles.litsItemWrapper}>
            <ul>
                {activeTab === 'recipes' && queryRecipesData.recipes?.length > 0 &&
                    <>
                    {queryRecipesData.recipes.map(recipe => (
                        <li key={recipe.id}>
                            <RecipePreview recipe={recipe} onDelete={()=>{}}/>
                        </li>)
                    )}
