...
export default function Home() {
  ...
  const [ingredientId, setIngredientId] = useState(
    "274df206-e919-4ba7-af37-fcd9ce558b63"
  );
  ...
  const handleSelectFavoriteRecipe = useCallback(
    async (recipesData) => {
      ...
      const matchedRecipes = matchRecipeWithFavorites(
        recipesData || recipes,
        favoritesData
      );
    ...
    const fetchRecipes = async () => {
      try {
        const response = await api.get(
          `/api/recipes?page=${page}&limit=10&category=${category.id}&area=${areaId}&ingredient=${ingredientId}`
        );
    ...
    <>
      ...
      <Container>
        {!isEmptyObject(category) && (
          <button className={style.btn} onClick={() => setCategory({})}>
        ...
        <Title>{isEmptyObject(category) ? "Categories" : category.name}</Title>
        <UnderTitle>
          {isEmptyObject(category)
          ...
          <div className={style["recipes-category"]}>
            ...
            <div>
              <div className={style["recipes-list"]}>
                <RecipeList
                  recipes={recipes}
                  addFavorite={handleSelectFavoriteRecipe}
                />
                {recipes.length === 0 && (
